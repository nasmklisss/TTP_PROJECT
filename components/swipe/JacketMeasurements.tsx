...
interface JacketMeasurementsProps {
  imageUri: string;
  height: number;
  gender: string;
}

export function JacketMeasurements({ imageUri, height, gender }: JacketMeasurementsProps) {
  const screenWidth = Dimensions.get('window').width;
  
  // Calculate measurements
  const calculateMeasurements = () => {
    let chest, sleeve, shoulder, waist, jacketLength;
    
    if (gender.toLowerCase() === 'male') {
      chest = height * 0.6;
      sleeve = height * 0.38;
      shoulder = height * 0.275;
      waist = height * 0.45;
      jacketLength = height * 0.52;
    } else {
      chest = height * 0.63;
      sleeve = height * 0.36;
      shoulder = height * 0.26;
      waist = height * 0.42;
      jacketLength = height * 0.47;
    }
    
    return { chest, sleeve, shoulder, waist, jacketLength };
  };
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
      <ThemedText style={styles.title}>Jacket Measurements</ThemedText>
      ...
      <View style={[styles.table, { width: screenWidth * 0.95 }]}>
        ...
        <TableRow label="Jacket Length" value={measurements.jacketLength} />
      </View>
    </ScrollView>
  );
}
